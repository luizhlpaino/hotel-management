...
using Requests = Application.Requests.Guest;
using Ports = Application.Ports.Guest;
using DTO = Application.DTO.Guest;
...
public class GuestManagerTests
{
    ...
    public async Task Should_Return_GuestNotFoundException_When_Guest_Is_Not_Found_With_Id()
    {
        ...
        Assert.IsNotNull(response);
        Assert.False(response.Success);
        Assert.AreEqual(ErrorCodes.GUEST_NOT_FOUND, response.ErrorCode);
    }
